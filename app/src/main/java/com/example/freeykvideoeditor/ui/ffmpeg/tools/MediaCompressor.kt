...
import com.example.freeykvideoeditor.utils.Log
...
class MediaCompressor(private val context: Context) {
    ...
    fun cancelAllOperations() {
        println("Canceling all ffmpeg operations")
        FFmpegKit.cancel()
    }
...
//        }
        ...
        if (!utils.supportedMediaType(mediaType)) { // not supported show error
            Toast.makeText(
                context, context.getString(R.string.error_unknown_filetype), Toast.LENGTH_LONG
            ).show()
            failureHandler()
            return
        }
        ...
        if (mediaInformation == null) {
            ...
            Toast.makeText(
                context, context.getString(R.string.error_invalid_file), Toast.LENGTH_LONG
            ).show()
            failureHandler()
            return
        }
        ...
        println("This is the output outputFileUri :: $outputFileUri")
        val command = "-i $inputSaf -ss 0 -t 5 -c:v copy -c:a copy $outputSaf"
        val prettyCommand = "ffmpeg -y -i $inputFileName  ${outputFile.name}"

        val ffmpegCommand =
            "-i $inputSaf -vf \"drawtext=text='Deepak':x=10:y=10:fontsize=24:fontcolor=white\" -c:a copy $outputSaf"
        ...
        FFmpegKit.executeAsync(command, { session ->
            ...
            if (!session.returnCode.isValueSuccess) { // failed
                if (!session.returnCode.isValueCancel) { // failure was not caused by a cancel
                    ...
                    Handler(Looper.getMainLooper()).post {
                        Toast.makeText(
                            context, context.getString(R.string.ffmpeg_error), Toast.LENGTH_LONG
                        ).show()
                    }
                    // save log
                ...
                println("This is the output location :: $outputSaf")
                println("This is the output outputFileUri :: $outputFileUri")
                if (settings.copyExifTags && ExifTools.isValidType(mediaType)) {
                    println("copying exif tags")

                    ExifTools.copyExif(
                        context.contentResolver.openInputStream(inputFileUri)!!, outputFile
                    )
                }
...
//                    txtProcessedTime.text = utils.millisToMicrowave(duration)
    ...
    fun compressFiles(
        activity: Activity, inputFilesUri: ArrayList<Uri>, callback: (uris: ArrayList<Uri>) -> Unit
    ) {
        ...
        iteratorFunction = @RequiresApi(Build.VERSION_CODES.Q)
        fun(i, error) {
            // base case
            if (i < inputFilesCount) {
                println("Processing %d of %d files ${i + 1} $inputFilesCount")
            ...
            if (i >= inputFilesCount || error) { // end of iterations
                ...
                if (settings.showStatusMessages && !error) {
                    ...
                    Handler(Looper.getMainLooper()).post {
                        println("Showing compression size toast message")
...
//                if (outputMediaType != Utility.MediaType.WEBM) {
//                    val maxBitrate = (settings.videoMaxFileSize / mediaInformation.duration.toFloat().toInt())
//                    println("Maximum bitrate for targeted filesize (%dK): %dk ${settings.videoMaxFileSize} $maxBitrate")
//
...
//        }
//        println("This is the param string :: $params")
